...
def read_product_json_openai_vectors() -> t.List:
    with open(config.DATA_LOCATION + "/product_vectors_openai.json") as f:
        product_vectors = json.load(f)
    return product_vectors
...
async def set_product_openai_vectors(product_vectors, redis_conn, products_with_pk):
    # iterate through products data and save vectors hash model
    for product in product_vectors:
        product_id = product["product_id"]
        product_pk = products_with_pk[product_id]
        ...
        await redis_conn.hset(
            ...
            mapping={
                ...
                "img_vector": np.array(product["img_vector"], dtype=np.float32).tobytes(),
                "text_vector": np.array(product["text_vector"], dtype=np.float32).tobytes(),
                "openai_text_vector": np.array(product["openai_text_vector"], dtype=np.float32).tobytes()
            })
...
async def load_all_data():
    ...
    if await redis_conn.dbsize() > 5000:
        ...
        vectors = read_product_json_openai_vectors()
        await set_product_openai_vectors(vectors, redis_conn, products_with_pk)
        ...
        await create_openai_hnsw_index(redis_conn, len(products), prefix="product_vector:", distance_metric="COSINE")
